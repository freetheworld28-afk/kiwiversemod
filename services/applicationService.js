...
function buildApplicationModal() {
  ...
  const experience = new TextInputBuilder()
    ...
    .setLabel('Previous moderation or staff experience')
  ...
  const why = new TextInputBuilder()
    .setCustomId('why')
    .setLabel('Why do you want to join KiwiVerse staff?')
    .setStyle(TextInputStyle.Paragraph)
    .setRequired(true)
    .setMaxLength(1000);
  ...
  const activity = new TextInputBuilder()
    ...
    .setLabel('How active can you be each week?')
    .setStyle(TextInputStyle.Paragraph)
    .setRequired(true)
    .setMaxLength(500);
...
async function startApplication(interaction, database) {
  ...
  if (pending) {
    return interaction.reply({ content: `You already have an active application (#${pending.id}).`, ephemeral: true });
  }
...
async function submitApplication(interaction, database) {
  ...
  if (existing) {
    return interaction.reply({ content: `You already have an active application (#${existing.id}).`, ephemeral: true });
  }
  ...
  const embed = new EmbedBuilder()
    .setColor(0xfee75c)
    .setTitle(`📝 Staff Application #${result.lastID}`)
  ...
  if (reviewChannel?.isTextBased()) {
    ...
    await db.run(
      'UPDATE applications SET review_channel_id = ?, review_message_id = ? WHERE id = ?',
      reviewChannel.id,
      reviewMessage.id,
      result.lastID,
    );
  ...
  return interaction.reply({ content: `✅ Application #${result.lastID} submitted successfully. You’ll be DM’d when staff review it.`, ephemeral: true });
}
...
async function showStatus(interaction, database) {
  ...
  const label = app.status === 'accepted' ? '✅ Accepted'
    : app.status === 'rejected' ? '❌ Rejected'
      : app.status === 'interview' ? '💬 Interview requested'
        : '⏳ Pending';
...
async function handleDecision(interaction, database) {
  if (!isReviewer(interaction.member)) {
    return interaction.reply({ content: 'Only authorized staff can review applications.', ephemeral: true });
  }
  ...
  if (!['pending', 'interview'].includes(app.status)) {
    return interaction.reply({ content: `Application #${id} has already been ${app.status}.`, ephemeral: true });
  }
  ...
  const title = newStatus === 'accepted'
    ? '✅ Your KiwiVerse application was accepted'
    : newStatus === 'rejected'
      ? '❌ Your KiwiVerse application was not accepted'
      : '💬 KiwiVerse staff want to interview you';
  const description = newStatus === 'accepted'
    ? `Congratulations! Your staff application **#${id}** has been accepted.`
    : newStatus === 'rejected'
      ? `Your staff application **#${id}** has been reviewed and was not accepted this time.`
      : `Your staff application **#${id}** has moved to the interview stage. Staff will contact you with next steps.`;
  ...
  if (newStatus === 'interview') {
    return interaction.followUp({ content: `💬 Application #${id} moved to interview.`, ephemeral: true });
  }
}
